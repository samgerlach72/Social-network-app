...
export class LoadStoryLambda{
    handler = async(json: LoadMoreItemsRequest<Status>): Promise<LoadFeedOrStoryResponse<Status>> => {
        ...
        try{
            let response = new LoadFeedOrStoryResponse(...(await new StatusService().loadMoreStoryItems(event.authToken, event.user, event.pageSize, event.lastItem)), true, undefined);
            return response;
        } catch(error) {
            let response = new LoadFeedOrStoryResponse<Status>(null, null, false, (error as Error).message);
            return response;
        }
    }
}

export const handler = new LoadStoryLambda().handler;